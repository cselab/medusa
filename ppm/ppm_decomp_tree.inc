      ...
      !                 below a certain tolerance (give by the variable: factor)
      !
      ...
      !
      !  Remarks      : what should we do with the factor ? 
     ...
     &   minboxsize,min_sub,max_sub,nsubs,info,pcost)
...
#include "ppm_define.h"
      ...
      REAL(MK):: mean_npbx,rmean_npbx,max_npbx,var_npbx,fdiff
      REAL(MK):: t0,factor
      ...
      factor = 0.01_MK ! one pct.
      factor = 0.20_MK ! 20 pct.
      factor = 0.10_MK ! 10 pct.
...
!--------------------------------------------------
     ...
     &                      MPI_SUM,ppm_comm,info)
         ...
         IF (nbox.GT.ppm_nproc) THEN
            !-------------------------------------------------------------------
            !  Compute the mean number of particle per box at the next level
            !-------------------------------------------------------------------
            mean_npbx = REAL(Npartg,MK)/REAL(lbox - fbox + 1,MK)

            !-------------------------------------------------------------------
            !  Compute the variance and maximum deviation from this mean value
            !-------------------------------------------------------------------
            var_npbx = 0.0_MK 
            max_npbx = 0.0_MK 
            DO kbox=fbox,lbox
               fdiff    = ABS(REAL(npbxg(kbox),MK) - mean_npbx)
               var_npbx = var_npbx + fdiff**2
               IF (fdiff.GT.max_npbx) max_npbx = fdiff 
            ENDDO
            var_npbx = SQRT(var_npbx/REAL(lbox - fbox + 1,MK))

            !-------------------------------------------------------------------
            !  If the variance is acceptable, then stop 
            !-------------------------------------------------------------------
            IF (var_npbx.LT.factor*mean_npbx) THEN
!           IF (max_npbx.LT.factor*Npartg) THEN
               lcontinue = .FALSE.
            ENDIF 
         ENDIF 
 ...
 400     CONTINUE
     ...
     &                     'reallocating the subs failed!',__LINE__,info)
      ...
      DO ibox=fbox,lbox
         nsubs = nsubs + 1
  !       cost(nsubs) = npbxg(ibox)
         DO k=1,ppm_dim
            min_sub(k,nsubs) = min_box(k,ibox)
            max_sub(k,nsubs) = max_box(k,ibox)
         ENDDO
      ENDDO
      ...
      !-------------------------------------------------------------------------
      !  Estimate the computational cost of each of the domains (based on the
      !  number of particles in the box compare to the mean value)
      !-------------------------------------------------------------------------
!--------------------------------------------------
!       rmean_npbx = REAL(nsubs,MK)/REAL(Npartg,MK)
!       DO i=1,nsubs
!          cost(i) = cost(i)*rmean_npbx
!       ENDDO 
!--------------------------------------------------
