      ...
      !     Revisions    :
      !-------------------------------------------------------------------------
      !     $Log: ppm_interp_p2m_renorm.f,v $
      !     Revision 1.5  2006/08/31 17:24:09  pchatela
      !     removed a quote.... pos
      !
      !     Revision 1.4  2006/08/31 17:19:56  pchatela
      !     Bugfixes
      !     Removed ghost contributions to the field_reno between the
      !     upstream and downstream boundaries.
      !     Condition for application of field_reno, have to be on the upstream or
      !     downstream boundary
      !
      !     Revision 1.3  2006/08/30 14:44:02  pchatela
      !     Several bug fixes
      !     allocation, in the ghost mapping: 2 sends, 2 inits, wrong pop order
      !
      !     Revision 1.2  2006/08/29 16:43:13  menahel
      !     bad bugs fixed.
      !
      !     Revision 1.1  2006/08/11 12:04:02  menahel
     ...
     SUBROUTINE ppm_interp_p2m_renorm_ss_2d(xp,Np,up,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#elif __KIND == __DOUBLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_ds_2d(xp,Np,up,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#endif
#elif __MODE == __VEC
#if   __KIND == __SINGLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_sv_2d(xp,Np,up,lda,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#elif __KIND == __DOUBLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_dv_2d(xp,Np,up,lda,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#endif
#endif
#elif __DIME == __3D
#if   __MODE == __SCA
#if   __KIND == __SINGLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_ss_3d(xp,Np,up,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#elif __KIND == __DOUBLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_ds_3d(xp,Np,up,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#endif
#elif __MODE == __VEC
#if   __KIND == __SINGLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_sv_3d(xp,Np,up,lda,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#elif __KIND == __DOUBLE_PRECISION
     SUBROUTINE ppm_interp_p2m_renorm_dv_3d(xp,Np,up,lda,topo_id,mesh_id,kernel, &
          & ghostsize,field_up,info)
#endif
#endif
#endif       
...
9998 CONTINUE
     ...
     ALLOCATE(field_reno((1-ghostsize(1)):(ndata(1,1)+ghostsize(1)),&
          ...
          &              (1-ghostsize(3)):(ndata(3,1)+ghostsize(3)),ppm_nsublist(topoid)))
     ...
     CASE(ppm_param_rmsh_kernel_mp4)
       ...
       IF(lda.EQ.4) THEN
           DO ip = 1,store_info(isub)
              ...
              PRINT*,'###################################################'
              PRINT*,'RENORMALIZATION NOT IMPLEMENTED FOR LDA = 4'
              PRINT*,'###################################################'
           END DO
       !------------------------------------------------------------------------
...
#ifdef __STUPIDMICHAEL
#error please dont define that michael is stupid
              field_up(1,ip10,ip20,ip30,isub) = field_up(1,ip10,ip20,ip30,isub) + &
                   & a10*a20*a30*up(1,iq)
              field_up(1,ip10,ip20,ip31,isub) = field_up(1,ip10,ip20,ip31,isub) + &
                   & a10*a20*a31*up(1,iq)
...
9997 CONTINUE
     ...
     DO isub=1,nsubs
        isubl = ppm_isublist(isub,topoid)
        IF (min_sub(3,isubl,topoid).LT.(min_phys(3,topoid)+0.5*dx(3))) THEN
            ...
            END DO
        END IF
        IF (max_sub(3,isubl,topoid).GT.(max_phys(3,topoid)-0.5*dx(3))) THEN
     ...
     CALL ppm_map_field_ghost(field_up,lda,topo_id,mesh_id,ghostsize,maptype, &
          & info)
     !CALL ppm_map_field_ghost(field_reno,topo_id,mesh_id,ghostsize,maptype, &
     !     & info)
#endif     
     ...
     CALL ppm_map_field_ghost(field_up,topo_id,mesh_id,ghostsize,maptype, &
          & info)
#elif __MODE == __VEC
     ! BUG? POPPING IN THE SAME ORDER AS PUSH? 
     CALL ppm_map_field_ghost(field_reno,topo_id,mesh_id,ghostsize,maptype, &
          & info)
     CALL ppm_map_field_ghost(field_up,lda,topo_id,mesh_id,ghostsize,maptype, &
          & info)
        ...
        IF (min_sub(3,isubl,topoid).LT.(min_phys(3,topoid)+0.5*dx(3))) THEN
            DO k=1,ghostsize(3)
                DO j=1,ndata(2,isubl)
                    ...
                    !  have to finish this
                    !-----------------------------------------------------
                    ! PC CHANGE: What if the unity samples to something finite
...
#if   __MODE == __SCA
#if   __KIND == __SINGLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_ss_2d
#elif __KIND == __DOUBLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_ds_2d
#endif
#elif __MODE == __VEC
#if   __KIND == __SINGLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_sv_2d
#elif __KIND == __DOUBLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_dv_2d
#endif
#endif
#elif __DIME == __3D
#if   __MODE == __SCA
#if   __KIND == __SINGLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_ss_3d
#elif __KIND == __DOUBLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_ds_3d
#endif
#elif __MODE == __VEC
#if   __KIND == __SINGLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_sv_3d
#elif __KIND == __DOUBLE_PRECISION
     END SUBROUTINE ppm_interp_p2m_renorm_dv_3d
#endif
#endif
#endif       
