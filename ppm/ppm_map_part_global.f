...
!        DO idom=1,ppm_nsubs(topoid)
     ...
     &                bcdef(6).NE. ppm_param_bcdef_periodic))   ) THEN
      ...
      !  IFS: Well, if we use Dirichlet b.c. we might just want to loose the
      !       particles outside of the domain. So just skip them in the map.
      !       This is also consistent with the map_partial where particles
      !       are also lost (there are no periodic sub images in this case).
      !-------------------------------------------------------------------------
!     IF (nlist2.GT.0) THEN
!        info = ppm_error_fatal
!        CALL ppm_error(ppm_err_part_unass,'ppm_map_part_global',  &
