      ...
      !                 rpackdata(:,:)_        (F) optional storage array
      !
...
#if      __MODE == __SCA           
           up(1:npart) = up(1:npart) + dt * dup(1:npart)
#elif    __MODE == __VEC
           up(1:lda,1:npart) = up(1:lda,1:npart) + dt * dup(1:lda,1:npart)
#endif           
           t  = t  + dt
...
#include "ppm_ode_rhsfunc_macro.h"
#if      __MODE == __SCA
              up(1:npart) = up(1:npart) + tau * dup(1:npart)
#elif    __MODE == __VEC
              up(1:lda,1:npart) = up(1:lda,1:npart) + tau * dup(1:lda,1:npart)
#endif
              t  = t  + tau
...
#include "ppm_ode_rhsfunc_macro.h"
              ...
              END DO
#if      __MODE == __SCA           
              up(1:npart) = up(1:npart) + 0.5_mk* dt * dup(1:npart)
#elif    __MODE == __VEC
              up(1:lda,1:npart) = up(1:lda,1:npart) + &
                   &0.5_mk*dt*dup(1:lda,1:npart)
#endif           
              ppm_ode_sent(mid) = 1
           CASE(2) 
              !-----------------------------------------------------------------
...
#include "ppm_ode_rhsfunc_macro.h"
              ...
              DO i=1,Npart
#if      __MODE == __SCA
                  !Vector notation: caused seg fault for me (Philippe)!?
                  !Mem requirements?           
                  !up(1:npart) = up(1:npart) + 0.5_mk* dt * dup(1:npart)
                  ...
                  !up(1:lda,1:npart) = up(1:lda,1:npart) + &
                  !     &0.5_mk*dt*dup(1:lda,1:npart)
                  up(1:lda,i) = up(1:lda,i) + 0.5_mk*dt*dup(1:lda,i)
#endif       
              END DO
              ppm_ode_sent(mid) = 2
           CASE(2) 
...
#include "ppm_ode_rhsfunc_macro.h"
              ...
              DO i=1,Npart
                 ...
                 bfr(3,i)     = dup(i) !k2
#elif    __MODE == __VEC             
                 bfr((2*lda+1):3*lda,i) = dup(:,i) !k2
#endif                 

#if      __MODE == __SCA
                 up(i)   = bfr(1,i) + 0.5_mk* dt * dup(i)
#elif    __MODE == __VEC                 
                 up(:,i) = bfr(1:lda,i) + 0.5_mk*dt * dup(:,i)
#endif                 
...
#elif    __MODE == __VEC                 
                 up(:,i) = bfr(1:lda,i) +  1.0_MK/6.0_MK*dt*                   & 
        &                (bfr((lda+1):2*lda,i) + 2.0_MK*bfr((2*lda+1):3*lda,i)  &
        &                             + 2.0_MK*bfr((3*lda+1):4*lda,i)+ dup(:,i))
#endif                 
