      ...
      !                 ppm_fdsolver_finalize.
      !                 where FFTs are conducted. The poisson equation is solved
      !                 in the Fourier space and the result transformed backward.
      ...
      !
      !  Input        : 
      !                  lda_fv      (I) size of leading dimension in vector case
      ...
      !                                               solve_slab: not used  
      !
      !                  mesh_ids(3) (I) mesh IDs on which the FFTs are performed
      ...
      !  Bugfix in scalar version by adjusting an argument in the call of the FFT
      !  routines
      !
...
/*
      ...
      bcdef(5)          = ppm_param_bcdef_freespace
      bcdef(6)          = ppm_param_bcdef_freespace
*/
     ...
     &       mesh_id_xpen_complex
      ...
      assign = ppm_param_assign_internal
          ...
          CALL ppm_mktopo(xp,Npart,Nm,decomp,assign, min_phys,max_phys,  &
...
#elif __DIM == __VFIELD
         CALL ppm_fdsolver_map(DATA_fv,lda_fv, topo_ids_tmp, mesh_ids_tmp, ghostsize, info)
#endif
...
!         topo_ids(1)  = topo_id_xpen
  ...
  !        IF(ppm_rank.EQ.1) THEN
  !        print*,'rank', ppm_rank
  !        print*,'Output',n,' x', FFT_x(:,2,1)
  !        print*,'mesh_tmp', lda
  !        ENDIF
...
!       iopt = ppm_param_dealloc
!       CALL ppm_alloc(DATA_fv, lda_DATA_fv, iopt,info)
     ...
     &                      ndata_xpen_complex, info)
      ...
      CALL ppm_mktopo(xp,Npart,Nm_com,decomp,assign, min_phys,max_phys,  &
...
#if   __DIM == __SFIELD
      CALL ppm_fdsolver_map(DATA_fv_com,topo_ids_tmp, mesh_ids_tmp, ghostsize, info)
#elif __DIM == __VFIELD
      CALL ppm_fdsolver_map(DATA_fv_com,lda_fv,topo_ids_tmp, mesh_ids_tmp, ghostsize, info)
#endif
...
!         print*, '*****************DATA_fv_trans xy' 
!         print*, 'ndata(idom)', ndata_ypen(:,idom)

#if  __DIM == __SFIELD
           
    !     IF(ppm_rank.EQ.1) THEN           
    !     print*,'rank', ppm_rank
    !     print*,'mesh_tmp transpose ', ndata_trans(:,idom)
    !     print*,'Output',n,' x before transpose ', DATA_fv_com(:,2,1,k)
    !     ENDIF
#endif
      ...
      !   print*,'rank', ppm_rank
      !   print*,'mesh_tmp transpose ', ndata_trans(:,idom)
      !   print*,'Output',n,' x transpose ', data_com(2,:,1)
      !   ENDIF
...
!          print*,'mesh_tmp y ', ndata_trans(:,idom)
!         print*,'rank', ppm_rank
!         print*,'Output',n,' FFT_xy forth', FFT_xy(:,1,1)
      ...
      !-------------------------------------------------------------------------
!         print*, '*****************DATA_fv_trans back xy' 
!         print*, 'ndata', ndata_ypen
...
!          print*,'mesh_tmp transpose ', ndata_trans(:,idom)
!          print*,'Output z transpose ', data_com
      ...
      !-------------------------------------------------------------------------
           ...
           !print*,'Output z ', FFT_xyz
           !print*,'mesh_tmp z ', ndata_trans(:,idom)
   ...
   &           'istart not allocated',__LINE__,info)
    ...
    &                        'istart not allocated',__LINE__,info)
         ...
         ENDIF
          ...
          !print*,'lda', ndata_trans(:,idom)
          !print*,'Output poisson ', ppm_rank, FFT_xyz(:,1,1)
...
#endif
      !print*,'Output bw  ', data_com
      !print*,'mesh_tmp bw ', ndata_trans(:,idom)
...
!          print*,'Output x transpose', FFT_xy
!          print*,'mesh_tmp x transpose', ndata_trans(:,idom)
 ...
 2000 CONTINUE ! CASE SLAB continues
      ...
      DO k=1,ppm_nsublist(ppm_field_topoid)
        ...
        DO l=1, ndata_xpen_complex(3,idom)
          ...
          !print*,'mesh_tmp x transpose', ndata_xpen_complex(:,idom)
          !print*,'Output x transpose', FFT_x
...
!      print*,'Output Inversed = Input ?'

 
!     lda_DATA_fv(1)= lda_fv
!      lda_DATA_fv(2)= Nm(1)
!      lda_DATA_fv(3)= Nm(2)
!      lda_DATA_fv(4)= Nm(3)
!      lda_DATA_fv(5)= nsublist
