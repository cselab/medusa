      ...
      !  of particles and decomposes space based on a mesh. If one wants particles,
     ...
     &               min_sub,max_sub,nsubs,info,ndom)
      ...
      INTEGER                           :: i,icut,jcut,kcut,nsrem,ncut,j
      INTEGER                           :: cutdim,constdim,rc,tx,ty,tz
     ...
     &        (decomp .EQ. ppm_param_decomp_cuboid)) THEN
          ...
          !-----------------------------------------------------------------
          !  Allocate memory for cut sizes in grid units
          !-----------------------------------------------------------------
          iopt = ppm_param_alloc_fit
          ldu(1) = ppm_dim
          ldu(2) = 1
              ...
              IF (constdim .NE. 1) THEN
                  ...
                  DO icut=1,nblocks(1)
                      ...
                      IF (ppm_dim .GT. 2) THEN
                          ...
                          ty = SUM(Npx(2,1:nblocks(2)))+nblocks(2)
