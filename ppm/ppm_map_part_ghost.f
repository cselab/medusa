      ...
      !  The arguments to ppm_map_part_ghost_put() changed; so updated the call to the routine.
      !
     ...
     &                                  maptype,info)
      ...
      IMPLICIT NONE
#if    __KIND == __SINGLE_PRECISION | __KIND == __SINGLE_PRECISION_COMPLEX | __KIND_AUX == __SINGLE_PRECISION
     ...
     &            'ghostsize must be >=0.0',__LINE__,info)
      ...
      IF     (maptype.EQ.ppm_param_map_ghost_get) THEN
         ...
         !----------------------------------------------------------------------
#if __KIND == __INTEGER | __KIND == __LOGICAL | __KIND == __SINGLE_PRECISION_COMPLEX | __KIND == __DOUBLE_PRECISION_COMPLEX
         info = ppm_error_error
