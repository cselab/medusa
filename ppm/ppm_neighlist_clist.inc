      ...
      !                 nuber, but just linear from 1 to ppm_nsublist!
      !
      !  References   :
      !
      !  Revisions    :
     ...
     &                'cell list array CLIST',__LINE__,info)
      ...
      DO idom=1,lda
          ...
          IF (ppm_dim .EQ. 2) THEN
              ...
              !-----------------------------------------------------------------
              !  Add safety margin to outer faces
              !-----------------------------------------------------------------
!              IF (ppm_subs_bc(1,jdom,ppm_topoid) .NE. 0) THEN
!                  xmin(1) = xmin(1) - ppm_myepsd
!              ENDIF
!              IF (ppm_subs_bc(2,jdom,ppm_topoid) .NE. 0) THEN
!                  xmax(1) = xmax(1) + ppm_myepsd
!              ENDIF
!              IF (ppm_subs_bc(3,jdom,ppm_topoid) .NE. 0) THEN
!                  xmin(2) = xmin(2) - ppm_myepsd
!              ENDIF
!              IF (ppm_subs_bc(4,jdom,ppm_topoid) .NE. 0) THEN
!                  xmax(2) = xmax(2) + ppm_myepsd
!              ENDIF
          ELSE
              !-----------------------------------------------------------------
...
!                  xmax(2) = xmax(2) + ppm_myepsd
!              ENDIF
!              IF (ppm_subs_bc(5,jdom,ppm_topoid) .NE. 0) THEN
!                  xmin(3) = xmin(3) - ppm_myepsd
!              ENDIF
!              IF (ppm_subs_bc(6,jdom,ppm_topoid) .NE. 0) THEN
!                  xmax(3) = xmax(3) + ppm_myepsd
!              ENDIF
          ENDIF
          ...
          IF (ppm_dim .EQ. 2) THEN
              ...
              !-----------------------------------------------------------------
!              IF (ppm_subs_bc(1,jdom,ppm_topoid) .NE. 0) THEN
!                  xmin(1) = xmin(1) - ppm_myepss
!              ENDIF
!              IF (ppm_subs_bc(2,jdom,ppm_topoid) .NE. 0) THEN
!                  xmax(1) = xmax(1) + ppm_myepss
!              ENDIF
!              IF (ppm_subs_bc(3,jdom,ppm_topoid) .NE. 0) THEN
!                  xmin(2) = xmin(2) - ppm_myepss
!              ENDIF
!              IF (ppm_subs_bc(4,jdom,ppm_topoid) .NE. 0) THEN
!                  xmax(2) = xmax(2) + ppm_myepss
!              ENDIF
          ELSE
              !-----------------------------------------------------------------
...
!              ENDIF
!              IF (ppm_subs_bc(5,jdom,ppm_topoid) .NE. 0) THEN
!                  xmin(3) = xmin(3) - ppm_myepss
!              ENDIF
!              IF (ppm_subs_bc(6,jdom,ppm_topoid) .NE. 0) THEN
!                  xmax(3) = xmax(3) + ppm_myepss
!              ENDIF
          ENDIF
#endif
...
!                  xmax(i)    = xmax(i) + cellsize(i)
!                  Nm(i,idom) = Nm(i,idom) + 1    ! ghost cell layers
                  Ngl(ppm_dim + i) = 1
              ENDDO
          ELSE                           ! DO NOT EXPLOIT SYMMETRY => ghost layers 
              DO i=1,ppm_dim             ! all around
!                 xmax(i)    = xmax(i) + cellsize(i)
!                 xmin(i)    = xmin(i) - cellsize(i)
!                 Nm(i,idom) = Nm(i,idom) + 2    ! ghost cell layers
                 Ngl(i) = 1
                 Ngl(ppm_dim + i) = 1
              ENDDO
          ENDIF
