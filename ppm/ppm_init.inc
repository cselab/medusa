...
#include "ppm_define.h"
     ...
     &         'Call MPI_Init before calling ppm_init !',__LINE__,info)
      ...
      IF(ppm_rank.EQ.0) WRITE(mesg,'(2A)') '*** This is the PPM library starting on ',cbuf(1:ILEN)
      IF(ppm_rank.EQ.0) CALL ppm_log('ppm_init',mesg,info)
      IF(ppm_rank.EQ.0) CALL ppm_write(ppm_rank,'ppm_init',mesg,info)
#else
     ...
     &           ,__LINE__,info)
          ENDIF 
          ppm_myepsd = 10.0D0**REAL(tolexp,ppm_kind_double)
