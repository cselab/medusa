      ...
      !  Bugfix: resolved bug ID 000013. The indices of lda are now explicitly set
      !  in the MIN function of the lcopy action.
      !
...
#include "ppm_define.h"
     ...
     &           'DATA',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
      IF (lalloc) THEN
         ALLOCATE(work(ldl_new(1):ldu_new(1),ldl_new(2):ldu_new(2),   &
     ...
     &           'WORK',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
      IF (lcopy) THEN
         ldc(1:3) = MAX(ldl_new(1:3),lda(1:3))
         ldd(1:3) = MIN(ldu_new(1:3),ldb(1:3))
         DO k=ldc(3),ldd(3)
            DO j=ldc(2),ldd(2)
               DO i=ldc(1),ldd(1)
