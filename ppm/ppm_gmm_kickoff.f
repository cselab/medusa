      ...
      !-------------------------------------------------------------------------
      !
      !  Purpose      : This routine starts the gmm by computing second
      !                 order accurate approximations of the distance to
      !                 the interface near it, i.e. on neighboring grid
      !                 points. Every sign reversal is interpreted as
      !                 the location of an interface! To use interfaces
      !                 represented by a non-zero level set, apply
      !                 shifting as pre-processing.
      !
      !  Input        : lda             (I) Only present for vector
      !                                     fields. Index saying which
      !                                     element of the vector
      !                                     contains the level function
      !                                     value.
      ...
      !                 thresh          (F) Threshold for values in narrow
      !                                     band on input. Any
      !                                     zero-crossing from a value
      !                                     larger than this will not be
      !                                     considered as interface. Set
      !                                     this to the absolute value of
      !                                     whatever the field outside the
      !                                     band is initialized to.
      ...
      !
      !  Input/output : fdata(...)      (F) Level set data. Either rank 3
      ...
      !                                     On out: total number of points
      !                                     adjacent to the interface.
      !                                     OPTIONAL. Has to be present
      !                                     if closest or ipts are present!
      !
      ...
      !                                     present! Not unique. The
      !                                     same point will occur
      !                                     multiple times.
      !                 closest(:,:)    (F) coordinates of the closest
      !                                     points on the interface from
      !                                     the mesh points in ipts. Not
      !                                     unique!
      !                                     OPTIONAL. Only computed and
      !                                     returned if present.
      !
      !  Remarks      : This routine creates second-order accurate data
      !                 near the interface and is needed to kick off a
      !                 reinitialize a level set or extend a function to
      !                 a narrow band.
      !
      !                 The order of the method is limited by the order
      !                 of the finite differences used in computing the
      !                 rhs of the interpolation system. Use higher
      !                 order FD to get higher order initialization.
      !
      !                 Tests have shown that storing shifted indices
      !                 (i.e. ip1 = i+1) is faster than using
      !                 i+1 in the array index directly. We thus use
      !                 this technique here.
      !
      !                 Maybe we shoud actually allocate ipos and copy
      !                 the stuff so it will survive ppm_gmm_finalize??
      !                 This will be easy: just change the pointer
      !                 assignment at the end of the routine to a
      !                 physical copy operation.
      !
      !  References   : Chopp:2001
      !
      !  Revisions    :
      !-------------------------------------------------------------------------
      !  $Log: ppm_gmm_kickoff.f,v $
      !  Revision 1.1.1.1  2006/07/25 15:18:19  menahel
      !  initial import
      !
      !  Revision 1.12  2005/07/14 19:58:13  ivos
      !  Added OPTIONAL argument chi for mesh node positions in distorted
      !  (mapped) meshes. For use with AGM for example.
      !
      !  Revision 1.11  2005/06/04 01:03:33  ivos
      !  bugfix. Module work pointer was not restored in the end. This
      !  is necessary in case phi is grown.
      !
      !  Revision 1.10  2005/06/01 05:18:39  ivos
      !  Added threshold for interface detection as argument. fixed minor bug
      !  in copy-out.
      !
      !  Revision 1.9  2005/05/10 04:48:45  ivos
      ...
      !  code cosmetics.
      !
      !  Revision 1.8  2005/04/27 01:06:11  ivos
      !  Convergence tests completed, cleaned up code, optmized code (Shark),
      !  and changed structure to allow faster compilation.
      !
      !  Revision 1.7  2005/04/21 04:46:12  ivos
      !  bugfix: array pointers were not used correctly. BEWARE: when setting
      !  a pointer to an array slab, the LBOUND and UBOUND are _not_
      !  inherited, but reset to standard values.
      !
      !  Revision 1.6  2005/03/16 06:20:08  ivos
      !  Several bugfixes. 1st order version is now tested. Moved all large
      !  data to the module.
      !
      !  Revision 1.5  2005/03/15 19:02:44  michaebe
      !  split up the data statement
      !
      !  Revision 1.4  2005/03/12 04:08:35  ivos
      !  Misc bug fixes.
      !
      !  Revision 1.3  2005/03/11 21:10:07  ivos
      !  Added thresholded extensions and closest point transform.
      !
      !  Revision 1.2  2005/03/11 04:17:02  ivos
      !  Added possiblity to for velocity extension and to get back
      !  the closest point transform.
      !
      !  Revision 1.1  2005/03/10 01:37:22  ivos
...
#include "ppm_gmm_2dpermut_data.inc"
     ...
     &            ppm_kind_double)
      ...
      DO jsub=1,ppm_nsublist(gmm_topoid)
          ...
          xhi = ppm_cart_mesh(gmm_meshid,gmm_topoid)%nnodes(1,isub)-1
          yhi = ppm_cart_mesh(gmm_meshid,gmm_topoid)%nnodes(2,isub)-1
          facx= 0.5_MK
          facy= 0.5_MK
          facxy= 0.25_MK
#if   __DIM == __3D
          zhi = ppm_cart_mesh(gmm_meshid,gmm_topoid)%nnodes(3,isub)-1
          facz= 0.5_MK
          facxz= 0.25_MK
          facyz= 0.25_MK
          facxyz= 0.125_MK
...
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i-1)*dx, &
!    &                               ppm_min_subs(2,isub,gmm_topoid)+(j-1)*dy, &
!    &                               ppm_min_subs(3,isub,gmm_topoid)+(k-1)*dz
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i)*dx, &
!    &                               ppm_min_subs(2,isub,gmm_topoid)+(j-1)*dy, &
!    &                               ppm_min_subs(3,isub,gmm_topoid)+(k-1)*dz
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i-1)*dx, &
!    &                               ppm_min_subs(2,isub,gmm_topoid)+(j)*dy, &
!    &                               ppm_min_subs(3,isub,gmm_topoid)+(k-1)*dz
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i)*dx, &
!    &                               ppm_min_subs(2,isub,gmm_topoid)+(j)*dy, &
!    &                               ppm_min_subs(3,isub,gmm_topoid)+(k-1)*dz
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i-1)*dx, &
!    &                               ppm_min_subs(2,isub,gmm_topoid)+(j-1)*dy, &
!    &                               ppm_min_subs(3,isub,gmm_topoid)+(k)*dz
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i)*dx, &
!    &                               ppm_min_subs(2,isub,gmm_topoid)+(j-1)*dy, &
!    &                               ppm_min_subs(3,isub,gmm_topoid)+(k)*dz
!                     WRITE(30,'(3E20.8)') ppm_min_subs(1,isub,gmm_topoid)+(i-1)*dx, &
...
#if   __KIND == __SINGLE_PRECISION
                              x = ppm_min_subs(1,isub,gmm_topoid)+(i-1)*dx
                              y = ppm_min_subs(2,isub,gmm_topoid)+(j-1)*dy
                              z = ppm_min_subs(3,isub,gmm_topoid)+(k-1)*dz
#elif __KIND == __DOUBLE_PRECISION
                              x = ppm_min_subd(1,isub,gmm_topoid)+(i-1)*dx
                              y = ppm_min_subd(2,isub,gmm_topoid)+(j-1)*dy
                              z = ppm_min_subd(3,isub,gmm_topoid)+(k-1)*dz
#endif
                          ENDIF
                          x0    = 0.0_MK
                          IF (PRESENT(chi)) THEN
...
!                     OPEN(30,FILE=cbuf,STATUS='REPLACE',ACTION='WRITE')
!                     WRITE(30,'(2E20.8)') ppm_min_subd(1,isub,gmm_topoid)+(i-1)*dx, &
!    &                               ppm_min_subd(2,isub,gmm_topoid)+(j-1)*dy
!                     WRITE(30,'(2E20.8)') ppm_min_subd(1,isub,gmm_topoid)+(i)*dx, &
!    &                               ppm_min_subd(2,isub,gmm_topoid)+(j-1)*dy
!                     WRITE(30,'(2E20.8)') ppm_min_subd(1,isub,gmm_topoid)+(i-1)*dx, &
!    &                               ppm_min_subd(2,isub,gmm_topoid)+(j)*dy
!                     WRITE(30,'(2E20.8)') ppm_min_subd(1,isub,gmm_topoid)+(i)*dx, &
!    &                               ppm_min_subd(2,isub,gmm_topoid)+(j)*dy
!                     WRITE(30,'(2E20.8)') rhs(1),rhs(2)
!                     WRITE(30,'(2E20.8)') rhs(3),rhs(4)
