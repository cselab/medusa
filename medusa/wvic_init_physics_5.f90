!-------------------------------------------------------------------------------
!          this is the same ring as in physics_4 but it also includes a
!         piston vortex to mimick the experiments (see Allen and Auvity)
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
SUBROUTINE wvic_init_physics_5
  ...
  REAL(mk)                 :: a0, b0, gamma
  REAL(mk)                 :: gamma_p, vrrad_p, vsrad_p
  ...
  END IF
  IF(piston_RE_ratio.LT.0.0_mk) THEN
     without_piston = .TRUE.
  ELSE
  ...
  !  roughly 2.6cm makeing it 2.8 now
  vrrad  = 0.014_mk
  ...
  !=================================================================
  ! PISTON RING PARAMETERS
  !-----------------------------------------------------
  !
  gamma_p = gamma * piston_RE_ratio
  vrrad_p = piston_radius
  !  vsrad_p = vsrad * SQRT(1.0_mk/(piston_RE_ratio*piston_vort_ratio))
  IF(without_piston) THEN
     !-- just a value
     vsrad_p = 1.0_mk
  ELSE
     vsrad_p = vsrad * SQRT(piston_RE_ratio/piston_vort_ratio)
  END IF
...
#include "wvic_put_a_ring.inc"
  IF(without_piston) GOTO 4321
  !-----------------------------------------------------
  !  create piston vortex
  !-----------------------------------------------------
  RING_CENTER(3) = max_physg(3) - 2.0_mk*vsrad
  vrrad = vrrad_p
  vsrad = vsrad_p
  gamma = -gamma_p
  radstr = 1.0_mk/vsrad**2
...
END SUBROUTINE wvic_init_physics_5
